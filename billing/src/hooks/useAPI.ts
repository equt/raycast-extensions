...
export type FixedSWRInfiniteKeyedMutator<Data> = <MutationData = Data>(
  data?: MutationData | Promise<MutationData | undefined> | MutatorCallback<Data>,
...
export function useAPI<T>(endpoint: string, options?: Options) {
  ...
  const { data, mutate, isLoading, isValidating } = useSWR<T>(
    params === null ? null : `https://api.equt.services${endpoint}?` + toURLSearchParams(params).toString(),
    (url: string) =>
      fetch(url, {
        headers: {
          Authorization: `Bearer ${getPreferenceValues<PreferenceValues>().TOKEN}`,
